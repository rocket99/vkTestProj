...
void TKPipelineManager::prepareAllPipelines(){
	TKPipelineManager::manager()->preparePipelineFromJson("basic_pipeline.json");
...
void TKPipelineManager::preparePipelineFromJson(const char *fileName){
	TKJsonPipeline *pipeline = TKJsonPipeline::createFromJson(fileName);
    if(pipeline != nullptr){
        std::string pipelineName = pipeline->pipelineName();
        TKPipelineManager::manager()->pipelineMap[pipelineName] = pipeline;
    }
}

void TKPipelineManager::addNewPipeline(TKJsonPipeline *pipeline, std::string name){
    if(pipeline == nullptr){
        return;
    }
    TKPipelineManager::manager()->pipelineMap[name] = pipeline;
}

VkPipeline TKPipelineManager::getPipeline(const std::string &pipelineName){
    std::map<std::string, TKJsonPipeline *>::iterator iter;
    iter = TKPipelineManager::manager()->pipelineMap.find(pipelineName);
    if(iter == TKPipelineManager::manager()->pipelineMap.end()){
        return VK_NULL_HANDLE;
    }
    return ((TKPipeline *)(iter->second))->pipeline();
}

TKJsonPipeline *TKPipelineManager::getTKJsonPipeline(const std::string &pipelineName){
    std::map<std::string, TKJsonPipeline *>::iterator iter;
    iter = TKPipelineManager::manager()->pipelineMap.find(pipelineName);
    if(iter == TKPipelineManager::manager()->pipelineMap.end()){
        return nullptr;
...
void TKPipelineManager::prepareBasicPipeline(){
    ...
    stencilOpState.compareMask = 1;
    stencilOpState.writeMask   = 1;
